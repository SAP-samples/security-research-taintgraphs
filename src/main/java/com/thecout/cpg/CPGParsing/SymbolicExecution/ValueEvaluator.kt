...
class ValueEvaluator(
    ...
    val nodeValues: MutableMap<Node, Any?> = mutableMapOf()
    ...
    fun setValueForNode(node: Node, value: Any?) {
        if (value is CouldNotResolve) {
            return
        }
    ...
    override fun visit(node: Node) {
        when (node) {
            is Expression -> setValueForNode(node, evaluate(node))
            is Declaration -> setValueForNode(node, evaluateDeclaration(node))
            else -> {
               // log.info("Cannot evaluate node of type ${node.javaClass.simpleName}")
    ...
    fun evaluate(expr: Expression?): Any? {
        ...
        when (expr) {
            ...
            is DeclaredReferenceExpression -> {
                ...
                return try {
                    evaluate(expressions.firstOrNull())
